...
\begin{figure}[H]
    ...
    \caption{Distribution of Coefficient of Variation (CV) for all vote estimates. The distribution is right-skewed with a mean of $\approx 0.46$. This shape indicates that for most "Bubble" candidates (left), the model is highly certain, while for "Safe" front-runners (right tail), the solution space is larger, resulting in higher variance.}
...
\begin{figure}[H]
    ...
    \caption{Global Correlation Analysis: Judge Scores vs. Estimated Vote Shares. Orange points represent eliminated contestants. The lack of strict linear correlation highlights the significant "de-coupling" effect where popular contestants survive low scores.}
