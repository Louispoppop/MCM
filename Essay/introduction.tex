...
"Dancing with the Stars" (DWTS) is a hit reality competition pairing celebrities with professional dancers. Each week, pairs are evaluated through a hybrid mechanism: judge scores from experts and fan votes from the audience. These two inputs are combined to determine which couple is eliminated, ultimately crowning a champion in the finale.
...
\begin{figure}[H]
    ...
    \label{fig:intro}
\end{figure}
Over 34 seasons, the show has utilized different aggregation methods—primarily Rank-based and Percent-based systems—to weigh these inputs. However, the clash between dance skill and star power has led to notable controversies, where popular contestants advanced despite low technical scores. This study analyzes these historical voting dynamics to quantify fan influence and proposes an optimized framework to better balance competitive fairness with viewer engagement.

\subsection{Restatement of the Problem}

The central objective of this research is to decode the decision-making dynamics within "Dancing with the Stars," specifically how professional evaluations interact with public preference to determine contest results. Given the unavailability of raw audience voting data, we must infer these hidden variables to rigorously assess the fairness of the competition. The specific objectives are defined as follows:
...
\begin{itemize}
    ...
    \item \textbf{Task 4: Policy Optimization and Strategic Recommendation.} We need to propose a novel system that optimizes the trade-off between technical guarantee and viewer engagement. Furthermore, we are tasked with compiling our insights into a concise memo for the producers, offering evidence-based guidelines for future seasons.
\end{itemize}
