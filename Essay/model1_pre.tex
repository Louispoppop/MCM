...
\begin{algorithm}[H]
    \caption{Iterative Inverse Solving for Latent Popularity $\boldsymbol{\beta}$}
    \begin{algorithmic}[1]
        \State \textbf{Input:} Observed Judge Scores $\mathbf{J}$, Elimination Outcomes $\mathcal{O}$
        \State \textbf{Output:} Latent Base Popularity $\hat{\boldsymbol{\beta}}$
        \State \textbf{Initialize:} $\boldsymbol{\beta}^{(0)} \leftarrow [1/N, \dots, 1/N]$
        \Repeat
        \State $k \leftarrow k+1$
        \For{each week $t = 1 \dots T$}
        \State Compute Expected Propensity $\boldsymbol{\mu}_t^{(k)}$ using Eq. (2)
        \State \textbf{E-Step (Sampling):} Draw $M$ samples $\{\mathbf{S}_t^{(m)}\} \sim \text{Dir}(\kappa \cdot \boldsymbol{\mu}_t^{(k)})$
        \State \textbf{Filter Step:} $\Phi_t \leftarrow \{ \mathbf{s} \in \{\mathbf{S}_t^{(m)}\} \mid L(\mathcal{O}_t | \mathbf{s}) = 1 \}$
        \If{$|\Phi_t| > 0$}
        \State Compute posterior mean $\bar{\mathbf{S}}_t \leftarrow \frac{1}{|\Phi_t|} \sum_{\mathbf{s} \in \Phi_t} \mathbf{s}$
        \State Infer implied popularity $\hat{\beta}_{i,t}^{\text{implied}}$ for each contestant $i$ using Eq. (5)
        \EndIf
        \EndFor
        \State \textbf{M-Step (Update):} $\beta_i^{(k+1)} \leftarrow \text{Average}(\hat{\beta}_{i,t}^{\text{implied}})$ over all separate weeks
...
The detailed procedure is outlined in Algorithm 1. In the \textbf{E-Step}, we generate $M$ potential voting scenarios for each week from the Dirichlet prior distribution. In the \textbf{Filter Step}, we identify and retain only those scenarios that satisfy the historical elimination constraints $\mathcal{O}_t$. Finally, in the \textbf{M-Step}, we compute the posterior mean of these valid samples and invert the mixture model to update our estimate of the latent base popularity $\boldsymbol{\beta}$.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{global_cv_distribution.png}
        \caption{Distribution of Coefficient of Variation (CV) for all vote estimates. }
        \label{fig:cv_distribution}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{ultimate_correlation.png}
