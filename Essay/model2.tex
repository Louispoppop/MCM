...
There are five key features serve as celebrity characteristics: industry, age, nationality, professional partner strength, and base popularity. 
...
\begin{itemize}
    \item \textbf{Fan Vote Share:} Both models achieve near-perfect $R^2$ ($\approx 0.99$). This consistency confirms that pre-existing popularity is the overwhelming, time-invariant driver of fan decisions~\cite{gershman2012teaching}.
...
In conclusion, the impact is \textbf{not} the same. Judges evaluate the \textit{performance} (multi-factor technical assessment), while fans largely validate the \textit{person} (single-factor popularity contest). Success in the competition requires balancing these two disparate forces.
