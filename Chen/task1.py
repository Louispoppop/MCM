...
def simulate_week(
    ...
    fan_prior: str = "dirichlet_uniform",  # 新增：选择分布
...
def estimate_fan_votes(
    ...
    for (season, week), g in df.groupby(["season", "week"], sort=True):
        ...
        week_results.append(WeekResult(int(season), int(
            week), method, feasible_rate, reproduced))
...
def main() -> None:
    ...
    out_estimates = os.path.join(os.path.dirname(
        __file__), "task1_fan_vote_estimates.csv")
    ...
    if not week_df.empty:
        overall_consistency = week_df["reproduced"].mean()
        avg_feasible_rate = week_df["feasible_rate"].mean()
        print("Consistency (overall reproduced rate):",
              f"{overall_consistency:.3f}")
        print("Consistency (avg feasible rate):", f"{avg_feasible_rate:.3f}")
