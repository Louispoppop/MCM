...
def plot_dynamic_mechanism():
    print(" -> 正在绘制自适应原理图 (Task4_Mechanism_Adaptive.png)...")
    ...
    plt.plot(x, y_linear, 'k--', linewidth=2, alpha=0.4, label='Original: Linear (Percent Rule)')
    plt.plot(x, y_tight, color='#3e7cd8', linestyle='-', linewidth=3, label='Scenario A: Tight Race (Higher K=80)')
    plt.plot(x, y_loose, color='#ee4d7a', linestyle='-', linewidth=3, label='Scenario B: Blowout (Lower K=20)')
    
    plt.axvline(center, color='gray', linestyle=':', alpha=0.6)
    plt.text(center, 0.0, ' Avg (10%)', color='gray', fontsize=10, ha='center', va='bottom')
    ...
    s_o1_ref = sigmoid_transformation(np.array([(1-x1_ref)/(n-1)]), n, k_tight)[0]
    y_tight_ref = s_t1_ref / (s_t1_ref + (n-1)*s_o1_ref)

    s_t2_ref = sigmoid_transformation(np.array([x1_ref]), n, k_loose)[0]
    s_o2_ref = sigmoid_transformation(np.array([(1-x1_ref)/(n-1)]), n, k_loose)[0]
    y_loose_ref = s_t2_ref / (s_t2_ref + (n-1)*s_o2_ref)
    # Draw reference line
    plt.plot([x1_ref, x1_ref], [0, y_tight_ref], color='gray', linestyle=':', alpha=0.5)
    plt.plot([x1_ref, x1_ref], [0, y_loose_ref], color='gray', linestyle=':', alpha=0.5)
    
    # Annotate Tight vs Linear
    diff_tight = y_tight_ref - y_loose_ref
    # Draw bracket/line
    plt.plot([x1_ref, x1_ref], [y_loose_ref, y_tight_ref], color='#9C86DA', linewidth=2)
    plt.scatter([x1_ref, x1_ref], [y_loose_ref, y_tight_ref], color='#9C86DA', s=15)
    ...
    s_o1_ref2 = sigmoid_transformation(np.array([(1-x2_ref)/(n-1)]), n, k_tight)[0]
    y_tight_ref2 = s_t1_ref2 / (s_t1_ref2 + (n-1)*s_o1_ref2)
    s_t2_ref2 = sigmoid_transformation(np.array([x2_ref]), n, k_loose)[0]
    s_o2_ref2 = sigmoid_transformation(np.array([(1-x2_ref)/(n-1)]), n, k_loose)[0]
    y_loose_ref2 = s_t2_ref2 / (s_t2_ref2 + (n-1)*s_o2_ref2)
    diff_loose = y_loose_ref2 - y_tight_ref2
    plt.plot([x2_ref, x2_ref], [y_tight_ref2, y_loose_ref2], color='#9C86DA', linewidth=2)
    plt.scatter([x2_ref, x2_ref], [y_tight_ref2, y_loose_ref2], color='#9C86DA', s=15)
    ...
    plt.title('Mechanism of Adaptive Meritocracy: Dynamic K-Factor (Normalized)', fontsize=15)
    ...
    plt.savefig('Task4_Mechanism_Adaptive.png', dpi=300)
    plt.close()
...
def plot_impact_analysis_refined(res_df, target_celeb="Bobby Bones"):
    ...
    print(f" -> Generating Impact Analysis Plot (Task4_Impact_{safe_name}_Refined.png)...")
    celeb_df = res_df[res_df['Celebrity'] == target_celeb].sort_values(['Season', 'Week'])
    ...
    if np.any(mask_norm):
        plt.scatter(np.array(weeks)[mask_norm], old_ranks[mask_norm], color='#9C86DA', s=80, label='Original (Percent)')
        plt.scatter(np.array(weeks)[mask_norm], new_ranks[mask_norm], color='#3e7cd8', marker='s', s=80, label='Adaptive System')
    
    mask_ne = is_non_elim
    if np.any(mask_ne):
        plt.scatter(np.array(weeks)[mask_ne], old_ranks[mask_ne], facecolors='none', edgecolors='#9C86DA', s=150, linestyle=':')
        plt.scatter(np.array(weeks)[mask_ne], new_ranks[mask_ne], facecolors='none', edgecolors='#3e7cd8', marker='s', s=150, linestyle=':')

    for i, (o, n, ne) in enumerate(zip(old_ranks, new_ranks, is_non_elim)):
        if n > o:
            alpha = 0.3 if ne else 0.8
            plt.arrow(i, o, 0, n-o-0.2, head_width=0.1, fc='#ee4d7a', ec='#ee4d7a', alpha=alpha)
            txt = f'( -{n-o} )' if ne else f'-{n-o}'
            plt.text(i+0.1, (o+n)/2, txt, color='#ee4d7a', va='center', fontsize=20, fontdict={'weight': 'bold', 'alpha': alpha})

    plt.gca().invert_yaxis()
    plt.title(f'Impact Analysis: {target_celeb} (Adaptive System)', fontsize=15)
    plt.ylabel('Weekly Rank', fontsize=12)
    plt.xlabel('Week', fontsize=12)
    plt.legend()
    plt.grid(True, axis='y', linestyle='--', alpha=0.5)
    plt.savefig(f'Task4_Impact_{safe_name}_Refined.png', dpi=300)
    plt.close()
...
def visualize_metrics_comparison_final_three_way(metrics_df):
    print(" -> 正在生成三方对比评分图 (Task4_Metrics_Final_ThreeWay.png)...")
    ...
    print(f"\n[三方评分结果]")
    print(f"Fairness: Rank={avg_fair_rank:.4f}, New={avg_fair_new:.4f}, Pct={avg_fair_pct:.4f}")
    print(f"Satisfaction: Rank={avg_sat_rank:.4f}, New={avg_sat_new:.4f}, Pct={avg_sat_pct:.4f}")
    
    # 2. 准备绘图数据
    labels = ['Fairness\n(Judge Correlation)', 'Satisfaction\n(Fan Correlation)']
    ...
    # 绘制三个柱子
    rects1 = ax.bar(x - width, rank_vals, width, label='Rule 1: Rank System', color='#3e7cd8', alpha=0.8)
    rects2 = ax.bar(x, new_vals, width, label='Proposed: Adaptive System', color='#ee4d7a', alpha=0.9)
    rects3 = ax.bar(x + width, pct_vals, width, label='Rule 2: Percent System', color='#9C86DA', alpha=0.8)
    
    ax.set_ylabel('Spearman Correlation', fontsize=12)
    ax.set_title('Comprehensive Comparison: Rank vs Proposed vs Percent', fontsize=14)
    ax.set_xticks(x)
    ...
    def annotate_bracket(x_start, x_end, y_h, diff, pct_diff, color, label_suffix):
        ax.plot([x_start, x_start, x_end, x_end], [y_h-0.02, y_h, y_h, y_h-0.02], color=color, lw=1.5)
        txt = f'{diff:+.3f}\n({pct_diff:+.1%})\n{label_suffix}'
        ax.text((x_start + x_end) / 2, y_h + 0.02, txt, ha='center', va='bottom', color=color, fontweight='bold', fontsize=9)
    ...
    annotate_bracket(x[0]-width, x[0], y_h_fair_1, diff_fair_rank, pct_fair_rank, color_fair_rank, "vs Rank")
    ...
    annotate_bracket(x[0], x[0]+width, y_h_fair_2, diff_fair_pct, pct_fair_pct, color_fair_pct, "vs Pct")
    ...
    annotate_bracket(x[1]-width, x[1], y_h_sat_1, diff_sat_rank, pct_sat_rank, color_sat_rank, "vs Rank")
    ...
    y_h_sat_2 = max(avg_sat_new, avg_sat_pct) + 0.08 # Stagger to avoid overlap with previous bracket
    annotate_bracket(x[1], x[1]+width, y_h_sat_2, diff_sat_pct, pct_sat_pct, color_sat_pct, "vs Pct")

    plt.tight_layout()
    plt.savefig('Task4_Metrics_Final_ThreeWay.png', dpi=300)
    plt.close()

if __name__ == "__main__":
    file_path = 'Cleaned_data_with_votes.csv'
    res_df, metrics_df = task4_simulate_new_system(file_path)
    res_df.to_csv("Task4_Simulation_Results.csv", index=False)
