...
def run_simulation_for_params(df, base_k, sensitivity):
    ...
    for _, g in grouped:
        ...
        if not np.isnan(s_score):
            satisfaction_scores.append(s_score)
...
def plot_sensitivity_heatmaps(res_df):
    print(" -> 正在绘制高对比度热力图...")
    output_path = GENERATED_FIGURES_DIR / 'task4_parameter_sensitivity_generated.png'

    pivot_fair = res_df.pivot(
        index='Base_K', columns='Sensitivity', values='Fairness')
    pivot_sat = res_df.pivot(
        index='Base_K', columns='Sensitivity', values='Satisfaction')
    pivot_comp = res_df.pivot(
        index='Base_K', columns='Sensitivity', values='Composite')
    ...
    # 1. Fairness Heatmap
    draw_heatmap(
        pivot_fair, axes[0], 'Metric: Fairness (Judge Correlation)', "YlGnBu")
    axes[0].set_ylabel('Base K (Steepness Baseline)')

    # 2. Satisfaction Heatmap
    draw_heatmap(
        pivot_sat, axes[1], 'Metric: Satisfaction (Fan Correlation)', "YlOrRd")
    axes[1].set_ylabel('')
        ...
        if 5 in bk_vals and 10 in sens_vals:
            ...
            for ax in axes:
                ...
                ax.add_patch(plt.Rectangle((col_idx, row_idx), 1, 1,
                             fill=False, edgecolor='blue', lw=3, clip_on=False))
    except ValueError:
        pass
...
if __name__ == "__main__":
    res_df = run_sensitivity_analysis(DATA_FILE)
